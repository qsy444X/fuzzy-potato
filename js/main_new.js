...
var favorites = JSON.parse(localStorage.getItem('travelFavorites')) || [];
...
function goBack() {
    if (window.history.length > 1) {
        ...
        var isIndex = window.location.pathname.endsWith('index.html') ||
            window.location.pathname === '/' ||
            window.location.pathname === '';
        var homePath = isIndex ? 'index.html' : '../index.html';
...
function initExplorePage() {
    ...
    function renderItems(data) {
        ...
        data.forEach(function (item) {
            var isIndex = window.location.pathname.endsWith('index.html') ||
                ...
                window.location.pathname === '';
            var path = isIndex ? 'pages/detail.html' : 'detail.html';
            var imgPath = isIndex ? item.image : '../' + item.image;
    ...
    function applyFilters() {
        ...
        console.log('搜索条件:', { category, priceRange, keyword });
        ...
        var filtered = attractions.filter(function (item) {
            ...
            if (keyword) {
                var searchText = (item.name + ' ' + item.city + ' ' + item.description).toLowerCase();
                matchKeyword = searchText.indexOf(keyword) !== -1;
            }
            var isMatch = matchCategory && matchPrice && matchKeyword;
            if (isMatch) {
                console.log('匹配景点:', item.name);
            }
            return isMatch;
        });
        ...
        if (resultCount) {
            resultCount.innerHTML = '共 <span>' + filtered.length + '</span> 个景点';
        }
...
function initDetailPage() {
    ...
    if (detailImage) {
        var isIndex = window.location.pathname.endsWith('index.html') ||
            window.location.pathname === '/' ||
            window.location.pathname === '';
        var imgPath = isIndex ? attraction.image : '../' + attraction.image;
    ...
    function renderFavorites() {
        ...
        favorites.forEach(function (item) {
            ...
            var isIndex = window.location.pathname.endsWith('index.html') ||
                ...
                window.location.pathname === '';
            var imgPath = isIndex ? item.image : '../' + item.image;
            card.innerHTML = '<img src="' + imgPath + '" alt="' + item.name + '">' +
                '<div class="card-body">' +
                '<h3>' + item.name + '</h3>' +
